...
for  gen=1:maxGEN
% 计算适应度的值，即路径总距离
[fval, sumDistance, minPath, maxPath,best] = fitness(distances, pop,Money_start,two,weather,cost);
% 轮盘赌选择
...
%figure 
%lot(minPathes, 'MarkerFaceColor', 'red','LineWidth',1);
...
for i=1:popSize
    water_now = 240;
    food_now = 240;
    money_now = Money_start - water_now*5 - food_now*10;
    %针对每一个种群的个体进行计算
    %在给定初始购买后进行第一阶段 行走阶段的计算
%当二进制码累加超过最短路距离时即为到达目的地
tempnum1 = 0;%记录路程数
tempnum2 = 1;%记录累加数
[dis1,p1] = floyd(maptest,1,55);
while tempnum1 < dis1
%     if tempnum1 == dis1-2
%         %到达矿山前经过村庄 此时补充补给到满
%        money_now = money_now - 20*(240-food_now) - 10*(240-water_now);
%        food_now = 240;
%        water_now = 240;
%        if money_now <0%防止没有资金购买的保险措施
%           money_now = money_now + 20*5 - 10*5;
%           food_now = food_now-5;
...
befalse = 0;
while tempnum2 < least_time-1
    %说明此时还可以挖矿
    if food_now <= 2*cost(2,weather(1,tempnum2+1))+ 3*cost(2,weather(1,tempnum2))|| water_now <= 2*cost(1,weather(1,tempnum2+1))+3*cost(1,weather(1,tempnum2))
       %说明此时需要补给 因为今天如果选择工作后不能及时去补给
       if food_now <= 2*cost(2,weather(1,tempnum2+1))|| water_now <= 2*cost(1,weather(1,tempnum2+1))
          %如果此时仍小于 这说明已经失败
          bool_success(1,i) = -1;
          break;
       end
       %但是此时如果未来有沙暴 则会多承受一轮移动 浪费时间在多余的路程也视为失败
       if weather(1,tempnum2+1) == 3
          bool_success(1,i) = -1;
          break;
       end
       ...
       if least_time - tempnum2 < 2 || weather(1,tempnum2) == 3
           ...
           if food_least2 - food_now >0 && water_least2 - water_now >0
               %此时都缺
               money_now = money_now - 20*(food_least2 - food_now) - 10*(water_least2 - water_now);
           ...
           elseif food_least2 - food_now >0 && water_least2 - water_now <=0
               ...
               money_now = money_now - 20*(water_least2 - water_now);
               water_now = water_least2;
           end
           %补给结束后直接跳出 进行返程
           cf = 1;
       ...
       money_now = money_now - 20*(food_least - food_now + 2*(cost(2,weather(1,tempnum2))));
       money_now = money_now - 10*(water_least - water_now + 2*(cost(1,weather(1,tempnum2))));
       food_now = food_least+ 2*(cost(2,weather(1,tempnum2)));
       water_now = water_least+ 2*(cost(1,weather(1,tempnum2)));
       %接着是补充在矿上还要工作的分量
       
       for ll = 1:earnmore_time
           money_now = money_now - 3*20*(cost(2,weather(1,tempnum2+2+ll)));
           food_now = food_now+3*cost(2,weather(1,tempnum2+ll+1));
           money_now = money_now - 3*10*(cost(1,weather(1,tempnum2+2+ll)));
           water_now = water_now+3*cost(1,weather(1,tempnum2+ll+1));
       end
       tempnum2 = tempnum2+1;%时间流逝
...
%             end
        ...
        if food_least - food_now >0 && water_least - water_now >0
               %此时都缺
               money_now = money_now - 20*(food_least - food_now-cost(2,weather(1,tempnum2))) - 10*(water_least - water_now-cost(1,weather(1,tempnum2)));
               food_now = food_least;
               water_now = water_least;
               tempnum2 = tempnum2 +1;
       elseif food_least - food_now >0 && water_least - water_now <0
               money_now = money_now - 20*(food_least - food_now-cost(2,weather(1,tempnum2)));
               food_now = food_least;
        elseif food_least - food_now <0 && water_least - water_now >0
               money_now = money_now - 20*(water_least - water_now-cost(2,weather(1,tempnum2)));
               water_now = water_least;
        end
...
if bool_success(1,i) == -1
    sumDistances(i) = 0;
     ...
     sumDistances(i) = money_now + food_now*2.5+ water_now*5;
end 
end
mmax = 0;
mmax_pos = 1;
